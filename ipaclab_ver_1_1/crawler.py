...
import time
import datetime
from ipaclab_ver_1_1.base_crawler import Params, BaseScraper

class ScriptSraper(BaseScraper):
    """번역문 클로링 클래스"""
    def __init__(self):
        super().__init__()
        self.data_dict={}
    ...
    def get_total_docs(self):
        ...
        total_docs = soup_main.find('div', class_='card-body').findChildren('div', class_='row')[1].findChildren('span')[0].text.replace('total docs : ', '')
    ...
    def do_crawler(self, start_page=0):
        ...
        for page in range(self.pages_count):
            ...
            res = BaseScraper.session.get(url=self.main_url, params=self.main_params, headers=self.headers)
            ...
            self.data_dict, dataset = self.script_crawler(tr, self.data_dict, self.tran_url, page, dataset)
    ...
    def script_crawler(self, tr, data_dict, tran_url, page, dataset):
        """스크립트 크롤링 메소드"""
        for i, td in enumerate(tr, start=1):
            ...
            res = BaseScraper.session.get(url=self.tran_url, params=self.crawling_params, headers=self.headers)
...
if __name__ == '__main__':
    p1 = ScriptSraper()
    p1.set_main_params(is_transcripted=2, is_assigned='voucher16')
    p1.do_crawler(start_page=0)
